from CodicePython.View.VistaInserisciDipendente import VistaInserisciDipendente
from PyQt5.QtWidgets import QVBoxLayout, QPushButton, QLabel, QLineEdit, QMessageBox, QWidget


class LoginNuovoDipendente(QWidget):

    def __init__(self, callback):
        super(LoginNuovoDipendente, self).__init__()
    ...
    def log(self):
        ...
        if username == "username" and password == "password":
            self.iscrivi_dipendente = VistaInserisciDipendente(callback=VistaInserisciDipendente.aggiungi_dipendente)
            self.iscrivi_dipendente.show()
        else:
